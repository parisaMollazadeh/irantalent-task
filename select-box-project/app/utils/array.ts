...
export const sortArrayBySelection = <T>(
  ...
  try {
    return array.sort((fItemA, fItemB) => {
      const firstSelected = selectedItems.some(
        (item) => item[field] === fItemA[field]
      );
      const secondSelected = selectedItems.some(
        (item) => item[field] === fItemB[field]
      );
